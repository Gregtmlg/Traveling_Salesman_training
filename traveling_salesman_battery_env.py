...
class TravelingSalesmanEnv(gym.Env):
    ...
    def __init__(self, max_steps=250, type_of_use="simple", nb_goals=5, action_k = 1, step_k = 1):
        ...
        self.battery = self.bluerov.get_battery()
        self.dist_to_base = 0
    ...
    def step(self, action):
        ...
        reward_battery_step = - (100 - self.battery) / 100
        # print("reward_battery = ", reward_battery_step)


        reward_current_action = self.__compute_reward()
        
        reward = reward_current_action - reward_before_action + (1 - self.step_k) + self.action_k * wpnt_choice_reward + reward_battery_step
        ...
        if self.rov_in_base and not done:
            done = True
            if (np.sum(self.goals[:,3]) == self.nb_goals):
                reward += self.step_max
            elif (np.sum(self.goals[:,3]) == 0):
                  reward -= 10
            else:
                reward += np.sum(self.goals[:,3]) * 200 * (self.battery / 100)

        if self.battery == 0:
            reward -= 10 * self.dist_to_base
            done = True
    ...
    def get_observations(self):
        ...
        self.battery = self.bluerov.get_battery()
        self.dist_to_base = self.ground.get_distance_to_base(self.current_position)
        ...
        'dist_to_base' : np.array([self.dist_to_base])
        }
    ...
    def __reset_parameters(self):
        ...
        self.step_counter = 0
        self.episode_reward = 0
        self.battery = self.bluerov.reset_battery()
        pass
